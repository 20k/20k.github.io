...
cl::buffer discretise(cl::context& ctx, cl::command_queue& cqueue, std::function<value<Type>(v3i)> func, tensor<int, 3> dim, float scale)
{
    auto kern = [](execution_context& ctx, buffer_mut<value<Type>> out, std::function<value<Type>(v3i)> func, tensor<int, 3> dim)
    {
    ...
    std::string str = value_impl::make_function(kern, "discretise", func, dim);
