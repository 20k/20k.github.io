...
This paper in theory lets us solve the TOV equations in a conformally flat spacetime, via (77, 78, + 79), but we've deliberately skipped this step, as the TOV equations are much easier to solve directly.
...
Note that this paper defines $\rho$ as what we've been calling $\epsilon$, and it calls $\epsilon$ a separate quantity we don't have (specific internal energy density). I'm going to keep our existing notation, and translate where appropriate, to try and staunch the insanity
