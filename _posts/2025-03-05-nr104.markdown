...
The conformally flatness assumption, as well as the maximal slice assumption, give us a familiar looking constraint to solve:
...
## Getting started: Solving for $\Phi$
...
The laplacian we need to solve for $\Phi_{tov}$, is (74) but with the constraints $\beta^i = 0, A_{ij} = 0, \epsilon_h = \epsilon$. This gives:

$$\Delta \Phi_{tov} = -2 \pi \Phi_{tov}^5 \epsilon_{tov}$$

This is easy enough to solve. We'll need to first discretise $\epsilon$ onto a grid, and then solve this in 3d
