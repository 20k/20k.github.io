...
Ie, we assume a constant $\epsilon$ through the star, then calculate this as a density. We'll have to guess our neutron star mass as well: the ratio of mass to radius is called the compactness factor $C$, and we can do $R=\frac{M}{C=0.06}$ as a guess. We're just looking for a vague ballpark range here
...
|-|-|-|
|Total energy density|$\epsilon$|$\rho$|
|Pressure|$P$ or $p$|$p$|
...
|$\epsilon_H$| Scalar matter source for the ADM equations|
...
This is a standard laplacian. We're going to go over the details of how to perform our full construction after we finish up our next step, which is solving for $\epsilon_H$. Whatever it is, it can't be that bad, right?

Todo: Londo (wesley?)

### Solving for $\epsilon_H$ is indeed that bad
...
The last thing to do is figure out what to do with our individual $\bar{\epsilon}_H$ terms. Luckily, you simply sum these, and make sure that an individual neutron stars contribution is $0$ outside of its radius. To make this super explicit:

$$\begin{align}
\bar{A}_{ij}^{sum} &= \sum^N_{k=1} \bar{A}_{ij}^{(k)}\\
F^{sum} &= \sum^N_{k=1} F_{(k)} \\
\bar{\epsilon}^{sum}_H &= \sum^N_{k=1} \bar{\epsilon}_H^{(k)}\\
\Delta u &= -\frac{1}{8} \Phi^{-7} \bar{A}_{ij}^{sum} \bar{A}^{ij}_{sum} - 2 \pi \Phi^{-3} \bar{\epsilon}^{sum}_H
\end{align}$$
...
This lets us solve for our ADM quantities:

|Name|ADM|Value|
|-|-|-|
|Metric Tensor|$\gamma_{ij}$|$$\Phi^4 \bar{\eta}_{ij}$$|
|Extrinsic Curvature|$$K_{ij}$$|$$\Phi^{-2} \bar{A}{ij}$$|
| Initial Adm Matter Source | $$\epsilon_H$$ | $$\Phi^{-8} \bar{\epsilon}_H$$ |
| Initial Adm Matter Source | $$S^i$$ | $$\Phi^{-10} S^i$$ |
| Initial Pressure | $P$ | $\Phi^{-8} \bar{P}$ |
| Initial Energy Density | $\epsilon$ | $\Phi^{-8} \epsilon$ |
...
If you're thinking *this sure seems like a lot to cram in one article*, in general relativity: the initial conditions are often significantly more complicated than the evolution equations. Today that's especially true, because its not going to be terribly complicated
...
Todo: Define $E^z$
Todo: I definitely need to harmonise notation. $\epsilon -> \mu$, $E^z -> \epsilon$
Todo: It would be better to calculate this from $S^i$, though it might be too annoying?
Todo: Use 22 to calculate $h$

Todo: tablify
Todo: We have a huge problem: The lorentz factor, and our conformal factor share notation. Consider using $W_l$ for lorentz
Todo: phi -> W

$$W^2 =\frac{\epsilon_H + P}{\epsilon + P}$$
$$u^0 = W$$
$$p_* = \rho \alpha u^0 e^{6\phi}$$
$$e_* = (p_0 E^z)^\frac{1}{\Gamma}\alpha u^0 e^{6\phi}$$

Note that $\Gamma=2$ is used in the gamma law equation of state: $(\Gamma-1) p_0 E^z$

$$\tilde{S}_i = e^{6\phi} \gamma_{ij} S^j$$

$$p_0 E^z = \frac{P}{\Gamma-1}$$

Where we've renamed this paper's $\epsilon$ to $E^z$, to avoid notational confusion
...
\partial_t \tilde{S}_k = &-\partial_i(\tilde{S}_k v^i) \\
                       &- \alpha e^{6\phi} \partial_k P - w h \partial_k \alpha \\
                       &-\tilde{S}_j \partial_k \beta^j + \frac{\alpha e^{-4\phi}\tilde{S}_i \tilde{S}_j}{2wh} \partial_k \tilde{\gamma}^{ij}\\
...
$v^i = -\beta + \frac{W^2 \alpha \tilde{\gamma}^{ij} \tilde{S}_j}{w h}$
...
We can solve #1 by using fixed point iteration with the initial guess $w=1$ (or perhaps, $w=p_*$) as this is a lorentz factor multiplied by density terms. #2 is more of a pain

The way this equation is structured is a little messy. Lets rearrange it a bit:

Todo: Make sure when implementing, that i perform the optimisation a^0 = 1

$$\begin{align}
A = (e^{-6\phi})^{\Gamma-1}\\
D = \frac{w^{\Gamma-1}}{w^{\Gamma-1} + A \Gamma e_*^{\Gamma} p_*^{\Gamma-2}}\\
w = \sqrt{p_*^2 + W^2 \tilde{\gamma}^{ij} \tilde{S}_i \tilde{S}_j D^2}\\
\end{align}
$$

Todo: If Gamma = 2, this equation set is kinda inappropriate due to the 1/Ws. we also get. I'm super dumb. Gamma = 2 is fine, past me was smrt. Todo: Delete the below

Correct: https://www.wolframalpha.com/input?i=%281+%2B+T%2F%28P%28w+K+%2F+P%29%5E%28G-1%29%29%29%5E-2

$$
D = \frac{1}{1 + A w^{1-\Gamma} \Gamma e_*^{\Gamma} p_*^{\Gamma-2}}\\
$$
...
$p_0$ and $E^z$ are often called the primitive variables, and recovering them requires $w$. Once you have that, you can calculate:

$$p_0 = \frac{e^{-6\phi} p_*^2}{w}$$

And

$$E^z = e_*^\Gamma p_*^{\Gamma - 2} (\frac{e^{-6\phi}}{w})^{\Gamma - 1}$$

Both of these non regular divisions have their limits at $0$

Other usefully optimised quantities are:

$$p_0 E^z = (e_* e^{-6\phi} \frac{p_*}{w})^\Gamma$$
...
1. $$\partial_t K += 4 \pi \alpha (S + \rho_s)$$
2. $$\partial_t \tilde{A}_{ij} += -8 \pi \alpha \tilde{S}_{ij}^{TF}$$
3. $$\partial \tilde{\Gamma}^i += -16 \pi \alpha \tilde{\gamma}^{ij} S_j$$

Things conspire very conveniently to avoid having to calculate infinite quantities at the singularity, as these terms are all finite at the singularity
