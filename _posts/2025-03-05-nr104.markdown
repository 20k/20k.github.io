...
Todo: Check that our parameter is matched to the star exterior, I believe it should be
...
Its worth noting the special case here when $\Gamma = 2$, in that the factor of $p_*$ drops out
...
There's a (genuinely) small problem: these source terms are singular at the singularity. The issue is that $\gamma_{ij}$ is undefined
...
Divisions by zero s u c c

Subnote: Projective reals (1/inf = 0)

Subnote 2: The actual projective reals \[a:b\]
