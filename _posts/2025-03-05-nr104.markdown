...
This is a much nicer way of calculating this, as $\Phi_{tov} = (\frac{r}{\bar{r}})^2$. No laplacians required. We'll also have to note: in the future, if we take the pressure at a radius, we'll need to perform a coordinate conversion if that radius is in isotropic coordinates: ie we need to keep on hand the function $r = f(\bar{r})$, which you should tabulate somewhere
