...
1. Convert everything into geometric units
2. Convert the mass of the sun, $1.988416 * 10^30 \mathrm{kg}$, into meters, lets say $M_m$
3. Divide your quantity in $\mathrm{m}^n$ (eg $\mathrm{m}^2$) by $(M_m)^n$

I'll be showing you the code for how to do this. One thing to note is that $K$ is frequently given in geometric units instead of physical units, so I'll flag up when things are wonky
