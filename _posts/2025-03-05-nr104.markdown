...
$$
e = \rho_0 + \frac{K \rho_0^\Gamma}{\Gamma - 1} \\
\epsilon = \frac{K \rho_0^{\Gamma-1}}{\Gamma - 1}\\
\rho = \rho_0 (1 + \frac{K \rho_0^{\Gamma-1}}{\Gamma - 1})\\
\rho = \rho_0 + \frac{K \rho_0^{\Gamma}}{\Gamma - 1}\\
\rho_0 = \sqrt[g]{\frac{p}{K}}\\
$$

Nope!

After some suffering, we get this:
...
$$A \frac{F[x-1] - 2 F[x] + F[x+1]}{h^2} + B \frac{-0.5 F[x-1] + 0.5 F[x+1]}{h}+ C F[x] = 0$$
...
https://www.wolframalpha.com/input?i=A+%28L+-+2+x+%2B+R%29%2Fh%5E2+%2B+B%28-0.5+L+%2B+0.5+R%29+%2F+h+%2B+C+x+%3D+0+solve+for+x

$$F[x] = \frac{2 A(F[x-1] + F[x+1]) + B h (F[x+1] - F[x-1])}{4A - 2Ch^2}$$

Right:

https://www.wolframalpha.com/input?i=A+%28x+-+2R+%2B+M%29%2Fh%5E2+%2B+B%28-x+%2B+R%29+%2F+h+%2B+C+x+%3D+0+solve+for+x

$$F[x] = -\frac{A F[x+2] - 2 A F[x+1] + B h F[x+1]}{A - Bh + Ch^2}$$

Left:

https://www.wolframalpha.com/input?i=A+%28M+-+2L+%2B+x%29%2Fh%5E2+%2B+B%28-L+%2B+x%29+%2F+h+%2B+C+x+%3D+0+solve+for+x
