...
Todo: the main segment below this needs to be scrapped, because its mostly wrong

# Bowen-York Type Initial Data for Binaries with Neutron Stars

The actual paper we're going to be implementing today is [this](https://arxiv.org/pdf/1606.04881) one. Its going to be a step up in complexity compared to things we've solved previously together. Here's our notational conversions
...
This paper in theory lets us solve the TOV equations in a conformally flat spacetime, via 77-79, but we've deliberately skipped this step, as the TOV equations are much easier to solve directly. Its also nice, because if you want to it becomes easy to plug in new equations of state, we're in a much more standard formalism
...
TODO: THEY ARE IN ISOTROPIC, WE ARE IN SCHWARZSCHILD

The 3d laplacian to solve for $\Phi_{tov}$, is (86). This is:

$$\Delta \Phi_{tov} = -2 \pi \Phi_{tov}^5 \epsilon_{tov}$$

We could also solve the 1d laplacian (77):

$$\frac{1}{r^2}\frac{d(r^2 \frac{d\Phi_{tov}}{dr})}{dr} = -2 \pi \Phi_{tov}^5 \epsilon_{tov}$$

Which is more usefully expressed as:

$$r\frac{d^2\Phi_{tov}}{dr^2} + 2 \frac{d\Phi_{tov}}{dr} + 2 \pi r \Phi_{tov}^5 \epsilon_{tov} = 0$$

Todo: we must reconfigure our tov - in schwarzschild coordinates - to isotropic coordinates

Todo: This analysis i think is wrong

That said, equation (82) is looking *very* interesting:

Todo: insert look back meme, possibly with wesleys face over the dudes face

$$M = 2 \pi \int_0^{r_0} r^2 \Phi_{tov}^5 \epsilon_{tov} dr$$

Where $r_0$ is the radius of our neutron star. Note that, from our TOV solution we already a-piori have $M$, $\epsilon_{tov}$, and $r_0$. So all we really need to do is differentiate this, to get:

$$\frac{dM}{dr} = 2 \pi r^2 \Phi_{tov}^5 \epsilon_{tov}$$

Which you'll notice is the same equation we had for TOV, but with a different scaling and a $\Phi_{tov}$. With this in hand, calculating $\Phi_{tov}$ (remembering that its a function of radius) becomes trivial. There's no loss in generality for our solution by doing this, and it is a free win. Things are simple for once! We've actually managed to significantly simplify the procedure outlined in this paper so far, which is nice[^nice]

[^nice]: I actually realised this entirely by accident while procrastinating from solving (86) or (77)
...
Solve the following equation for $\Phi_{tov}$, with $r_0 = R$, and $\epsilon_{tov} = \epsilon$
