...
$$e_* = (p_0 \epsilon)^\frac{1}{\Gamma}\alpha u^0 e^{6\phi}$$

Note that $\Gamma=2$ is used in the gamma law equation of state: $(\Gamma-1) p_0 \epsilon$

$$\tilde{S}_i = e^{6\phi} \gamma_{ij} S^j$$

$$p_0 \epsilon = \frac{P}{\Gamma-1}$$
...
$p_0$ and $\epsilon$ are often called the primitive variables, and recovering them requires $w$. Once you have that, you can calculate:
...
$$p_0 \epsilon = (e_* e^{-6\phi} \frac{p_*}{w})^\Gamma$$
