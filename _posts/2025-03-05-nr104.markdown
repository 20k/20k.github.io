...
Ie, we assume a constant $\epsilon$ through the star, then calculate this as a density. This is at least in the ballpark. From there, you can simply bruteforce over a range of values, eg $\epsilon/10$ -> $\epsilon*100$

There's a teeny small problem. Neutron star ADM masses are not unique. You see, the graph of radius vs mass looks like this:



## Key takeaways from solving TOV

At the end of this process, we've gained the ability to describe the following variables of our neutron star: rest mass density $\rho_0$, energy $\epsilon$, ADM mass $M$, and pressure $P$. Keep this in mind as we carry on forwards
