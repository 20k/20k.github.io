...
The root variables we derive BSSN from for ADM are still $\gamma_{ij}$, but instead of $\pi_{ij}$ we use the extrinsic curvature tensor $K_{ij}$, TODO I HAVE NO IDEA IF THEY'RE THE SAME. We still keep the same gauge variables $\alpha$, and $\beta^i$, and their definition is unchanged. Conformal variables in general have a tilde over them, and have their indices raised and lowered with the conformal metric tensor
...
The last one, $\tilde{\Gamma}^i$, is analytically equal to the RHS, but is evolved dynamically. This is because $\partial_k \tilde{\gamma}^{mn} \tilde{\Gamma}^i_{mn}$ is too numerically unstable TODO: CITE
