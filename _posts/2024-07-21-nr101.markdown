...
This article is not going to assume you know all that much about general relativity, beyond how to manipulate indices
...
The two ADM variables are:

$\gamma_{ij}$ the metric tensor, and $\pi_{ij}$ the generalised conjugate momentum

One of the insights of the BSSN formalism is that we can perform a 'conformal decomposition' of these variables. The conformal metric tensor is called $\tilde{\gamma}_{ij}$, and is related to the regular metric tensor by a conformal transform. There are three conformal transforms I know of which are in use:
...
Ie, they all define the same conformal metric. To calculate them is as follows:
...
The $\phi$ version was the first to be used, but it tends to infinity at the singularity which makes it not ideal. The second version to be used was $\chi$ (range, [0,1]) which tends to 0 at the singularity, and is relatively widespread in more modern code. Lastly $W^2$ entered the picture - ostensibly it has better convergence [TODO PAPER LINK] than $\chi$, but the really neat part about it is that renders the equations of motion of BSSN in a much less singular form. It also does not suffer from some of the issues that X has, like unphysical negative values

Because of this, the $W^2$ formalism is clearly the best choice for us. Do note that some papers interchange notation here unfortunately, and eg $\phi$ is sometimes used to mean *any* conformal factor

## Variables

The root variables we derive BSSN from for ADM are still $\gamma_{ij}$, but instead of $\pi_{ij}$ we use the extrinsic curvature tensor $K_{ij}$, TODO I HAVE NO IDEA IF THEY'RE THE SAME. We still keep the same gauge variables $\alpha$, and $\beta^i$, and their definition is unchanged. Conformal variables in general have a tilde over them, and have their indices are raised and lowered with the conformal metric tensor

The BSSN variables are then as follows:
...
The last one, $\tilde{\Gamma}^i$, is analytically equal to $\tilde{\gamma}^{mn} \tilde{\Gamma}^i_{mn}$, but is evolved numerically instead. This is because $\partial_k \tilde{\gamma}^{mn} \tilde{\Gamma}^i_{mn}$ is too numerically unstable TODO: CITE

We end up with the following constraints, that ideally are satisfied for all time:
...
$$
\tilde{\Gamma}^i - \tilde{\gamma}^{mn} \tilde{\Gamma}^i_{mn} = 0
$$
...
Here are the common gauge conditions we'll run into:
...
This is quite common in these tests, where
...
So, as written, the BSSN equations of motion are - drumroll - very numerically unstable. I'll summarise two decades of the conventional wisdom from stability analysis here:
