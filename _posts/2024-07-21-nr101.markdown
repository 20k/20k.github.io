...
$^{TF}$ is used to denote the trace free part of a tensor. $Q_{ij}^{TF} = Q_{ij} - \frac{1}{3} g_{ij} g^{mn} Q_{mn}$. Note that for some scalar $A$, $(AQ)^{TF} == A(Q^{TF})$, and it does not matter if you use the conformal metric, or the non conformal metric as the metric tensor here - as the conformal factor cancels out
...
3. The variables which have second derivatives taken of them are: $\tilde{\gamma}_{ij}$, $\alpha$, $\beta^i$, and $W$, and so require precalculated first derivatives
...
The BSSN equations do not specify the evolution equations for the lapse and shift, as these are arbitrary. We must pick appropriate gauge conditions for our problem
...
The self advection terms for both lapse and shift are often disabled, ie $\beta^m \partial_m \alpha$ and $\beta^j \partial_j \beta^i$
...
Note/todo, the substitution of $\Gamma$
...
Enforcing the other constraints is unnecessary in this article, so we'll save them for the future
