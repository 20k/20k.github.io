...
The right hand side terms in the paper correspond to matter, and are neglected here. We'll be getting back to the first paper linked above in the future, in a *truly* terrible amount of detail
...
void upscale_buffer_with_boundary(execution_context& ctx, buffer<valuef> in, buffer_mut<valuef> out, literal<v3i> in_dim, literal<v3i> out_dim, literal<valuef> boundary)
{
    ...
    ///bog standard trilinear interpolation
...
The derivatives at the boundary will be one sided derivatives. All waves have a speed of $v=1$, except for the gauge variables which have a speed of $v=\sqrt{2}$ in some circumstances. Todo: Cite
...
The structure of the equations we're using are already adapted to simulating binary black holes by design (surprise!) luckily, but older sets of equations were very numerically unstable for these problems. We're skipping well over 40 years worth of research in this article, and its still a very active field
...
1. The $\beta^k \partial_k$ terms are frequently dropped, as what they compensate (a zero speed mode) for is frequently addressed implicitly via kreiss-oliger dissipation (as it inherently dissipates away zero lapse points) and clamping
...
In general, momentum constraint damping dissipates away the extrinsic curvature - which tends to remove energy from the simulation
...
Trying to simulate with the above set of modifications at our resolution is not going to work, and instead today we're going to focus on trying to reproduce a specific result: todo. The no spin test case from [this paper](https://arxiv.org/pdf/gr-qc/0610128). Here, I'll lay out the test parameters:
...
With all of this together, we're able to replicate - close enough - the original paper, and we have now smashed black holes together pretty successfully. There are still clearly some inaccuracies however, which I'll go over now

# Discussion

Todo: Crank down less on the conspiracy theories here, focus more on the positives
