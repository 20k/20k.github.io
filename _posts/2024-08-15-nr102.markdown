...
[page 37](https://indico.cern.ch/event/505595/contributions/1183661/attachments/1332828/2003830/sperhake.pdf)
...
For our sim, we'll have the following test parameters

```c++
resolution = {213, 213, 213}
width = 20
h = (width / (resolution.x - 1))
```

By the end of this article, we will be close enough to demonstrate that the sim we've built isn't totally invalid
...
With the above modifications enabled, our sim goes from blowing up immediately, to successfully managing to simulate a collision at roughly T=130. Because we're more interested in the paths that the black holes take for the moment, rather than the specific collision time, I'm just eyeballing the numbers here
