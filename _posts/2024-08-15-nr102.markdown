...
In general, exactly reproducing other people's results is an absolute nightmare, and is virtually impossible in the general case. While it isn't hard to get close, the field has a strong issue with being underdocumented, and many of the larger simulators simply do not publish enough information to replicate their results. I also have some suspicions about the moving punctures technique, which we'll get into when its relevant

I'm going to walk us through how to take our sim from self destructing immediately, to successfully completing over an orbit

## First off, lets test the standard modifications

With the above modifications enabled, our sim goes from blowing up immediately, to self destructing at T=whatever. We can see that it exhibits the following features: todo

Experience tells me that the first problem to tackle is the christoffel symbol constraint

### Christoffel symbol errors, round 2

We're going to ditch the traditional christoffel symbol enforcement, and instead use two key other constraint handling techniques.

The first comes from this paper:

todo

The second comes from this paper[^papersketch]:
...
Running our code with both of these equations gets us a lot further:
...
For us, 2 and 3 are the problem. We're going to address kreiss-oliger in a slightly unusual way, and there's going to be a heavy tangent about kreiss oliger dissipation in general
...
The tricky part is that while we use scalar constants for damping factors, many of these factors are actually *not* grid resolution independent, and should really scale with some factor of the grid resolution
...
I have some questions about efficiency of the moving punctures technique as a whole. As mentioned previously, the puncture is a true discontinuity, and differentiating across it is invalid. This (allegedly) is a major source of constraint violations, though I've seen suggestions that the event horizon itself is also a source of constraint violations
