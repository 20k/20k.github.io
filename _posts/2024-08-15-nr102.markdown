...
I'm going to present you with the easiest set of initial conditions to implement, and we're going to really dig into it. Some discussion about alternatives is at the end of this article
...
1. This doesn't actually make black holes, it makes objects that collapse into black holes
...
This technique solves for $\gamma_{ij}$ and $K_{ij}$, and leaves us to guess $\alpha$ and $\beta^i$. It falls under the class of initial conditions known as 'conformally flat'. This means it decomposes the metric tensor into a flat conformal metric, and a conformal factor $\psi$, and decomposes the extrinsic curvature $K^{ph}_{ij}$ into a trace $K=0$, and a trace free part which this paper calls $K_{ij}$ (and other papers often call $$\tilde{A}_{ij}$$). The notation for numerical relativity has changed a lot over the years
...
$m_i$ bare mass of each black hole
$P_i$ the momentum vector of each black hole
$S_i$ the spin (angular momentum) of each black hole

Note that bare mass is not the same thing as mass, it only correlates to it. A black hole's spin contributes to its mass as well. On top of this, in the general case, the mass of a black hole in a binary pair isn't possible to define: see the end of this article for more details

## Extrinsic curvature

This is very easy. For every black hole in our spacetinme, we calculate the quantity (5), and then sum all of them. I then store this in a single buffer, $K^{ab} K_{ab}$ as a scalar
...
Where $u > 1$. We're going to deviate a little from this, and instead calculate the correction as:

$$\psi = \frac{1}{\alpha} + u + 1$$

Where $u > 0$. This is because you have a lot more precision to work with for floating point near zero vs near $1$, so this lets us solve for much smaller corrections without needing to resort to double precision. This doesn't actually change the structure of any of the equations we solve, it only changes the boundary condition

### Solving laplacians

Equations (9) + (10) are a classic laplacian, and I'm now going to present to you how to solve it. We're extremely lucky in that we're going to be running this on a GPU, so we don't have to do anything too fancy to solve this very quickly
