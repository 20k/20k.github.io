...
2. We use `still_going` parameter. This sets an atomic to `1` (ignore the `0`, its an address offset) if the change in cell value is greater than an error tolerance. This is so that we can dynamically terminate when we've converged enough
