...
4. This works for any number of black holes, but astrophysically there's virtually no way to have a 3 body interaction. Funwise, we'll go nuts when we get some better rendering in place
...
$m$ bare mass parameter of a black hole
...
To reiterate notation, I'm going to be using the notation $\bar{A}^{ij}$ to mean what this paper calls $K^{ij}$, because its simply too confusing otherwise. I'm also going to be using $\bar{\gamma}_{ij}$ to mean the conformally flat metric instead of $g_{ab}$, for the same reason. Note that $\bar{\gamma}_{ij}$ and $\tilde{\gamma}_{ij}$ are not the same, there's a lot of quite similar notation here

So far, we have the following variable definitions:

$$
\begin{align}
&\bar{\gamma}_{ij} &&= \tilde{\gamma}_{ij} \psi^{-4} = \delta_{ij} = I \\
...
Where $\epsilon$ is the levi civita symbol[^lcs]. Note that the levi civita symbol, and levi civita tensor have the same notation in the literature and are frequently freely mixed, so its not especially clear what's going on. Here, I believe this is specifically the *symbol*
...
Code wise: This is straightforward to implement:
...
Note that $r$ in the bottom term is a vector representing our current grid cell's world position. This is twice the distance of our coordinate from our $i$'th black hole. The $\alpha$ here has nothing to do with the gauge variable lapse also notated by $\alpha$

We're going to deviate a little from this definition, and instead calculate the correction as:

$$\psi = \frac{1}{\alpha} + u + 1$$

Where $u > 0$. This is because you have a lot more precision to work with for floating point near $0$ vs near $1$, so this lets us solve for much smaller corrections without needing to resort to double precision. This means our boundary condition will be slightly different (we pick $u=0$, instead of $u=1$)
...
This is the slightly more complex version of the equation. Instead, we'll implement the more modern, equivalent form from [this](https://arxiv.org/pdf/1606.04881) paper (18), or [this](https://www.worldscientific.com/doi/pdf/10.1142/S2010194512004321) paper (13), because that's where we'll end up eventually anyway, and because its simpler

Their equation:

$$\begin{align}
&\Delta \Phi + \frac{1}{8} \Phi^{-7} \bar{A}_{ij} \bar{A}^{ij} = 0 \\
\end{align}$$

Notational connections, and flattening the laplacian:

$$\begin{align}
&\Delta \Phi = \Delta u \\
&\Phi = \frac{1}{\alpha} + u + 1\\
\end{align}
$$
...
$$\begin{align}
&\Delta u + \frac{1}{8} (\frac{1}{\alpha} + u + 1)^{-7} \bar{A}_{ij} \bar{A}^{ij} = 0\\
\end{align}
$$

The right hand side terms correspond to matter, and are neglected here. We'll be getting back to the first paper linked above in the future, in a *truly* terrible amount of detail
...
Luckily, we're in GPU-land, and so the techniques are both simpler and significantly more performant. If you already know how to solve a laplacian, there is absolutely nothing special in this section and you may skip it
...
For us, $F(u) = -\frac{1}{8} (\frac{1}{\alpha} + u + 1)^{-7} \bar{A}_{ij} \bar{A}^{ij}$
...
The first is solved by relaxation: Ie taking our old guess, and our new guess, and taking some fraction between the two: `mix(u, nextu, 0.9f)`. This unfortunately reduces the speed of convergence

With that in mind, lets have a look at our first very basic laplacian solver!
                        ...
                        buffer_mut<valuei> still_going, literal<valuef> relax)
{
    ...
    //cfl is 1 + 1/alpha, in is our guess for u
...
2. We use a `still_going` parameter. This sets an atomic to `1` (ignore the `0`, its an address offset) if the change in cell value is greater than an error tolerance. This is so that we can dynamically terminate when we've converged enough
3. We use a linear id, and then convert that to our 3d id. This is to avoid significant dead threads at the edge of our domain
...
This means, we can simply update all red cells, then all black cells in an alternating fashion. Because the adjacent cells are one step into the future compared to the naive scheme, this is actually roughly a free 2x speedup in terms of iteration speed
...
$$u_{k+2} = \frac{D(u_{k+1})}{6} - \frac{F(u_k)h^2}{6}$$

This does not affect convergence in practice, but its worth keeping in mind that this is slightly nonstandard. Code wise, this is a simple change, and there are only two major differences:
...
Sometimes, the code is simpler than the explanation! Luckily, there is no race condition with respect to $F(u)$, as we're the only thread that both reads and modifies that value - otherwise we'd have to use two buffers again
...
None of the exact parameters here are particularly important: it only matters that the last grid resolution is the one we actually want to solve for. As usual, I'm pulling out the interesting snippets here, and the full code is available at TODO: EXACT LINK TO LAPLACIAN STUFF
...
$f$ is our value for whatever field we're radiating, $x_i$ is the cell's position in world coordinates, $v$ is the wave speed, $f_0$ is the asymptotic value, and $r$ is the distance from the origin in world coordinates. $\frac{\partial f}{\partial x_i}$ is the standard directional derivative of the function in each direction $\partial x_i$. This is applied to every tensor component separately, as if they were independent fields
...
Note that in my experience, Sommerfeld boundary conditions tend to lead to a gradual increase in ADM mass over time with a very close boundary, making it somewhat unsuitable for long lived sims for us. More complex boundary conditions are a future topic

There are two other boundary conditions we need:

1. Kreiss-Oliger dissipation. In our case, we'll progressively reduce the order of dissipation as we approach the boundary
2. Derivatives. Here, we reduce the order of derivatives as we approach the boundary, and use one sided derivatives *on* the boundary
...
The structure of the equations we're using are already adapted to simulating binary black holes by design (surprise!) luckily, but older sets of equations were numerically unstable for these problems
...
$\eta$ is a free parameter that represents damping: In general it is set to $\frac{2}{M}$, but it can also take fairly arbitrary values from $1$ to $>10$, or even vary spatially or by time. This parameter has a heavy influence on the paths of the black holes
...
1. The $\beta^k \partial_k$ terms are frequently dropped, as what they compensate (a zero speed mode) for is frequently addressed implicitly via kreiss-oliger dissipation (as it inherently dissipates away zero lapse points)
...
3. These gauge conditions only work for relatively equal mass compact object collisions. For cosmological simulations, you'd again need different gauge conditions - there's no one size fits all condition
...
I wish there was a better way to do tensors in C++
...
### Christoffel symbol errors (classic)

The traditional damping schemes are as following:

[page 37](https://indico.cern.ch/event/505595/contributions/1183661/attachments/1332828/2003830/sperhake.pdf) (which I'll call sigma damping)

$$
\partial_t \alpha = \partial_t \alpha - \sigma \mathcal{G}^i \partial_m \beta^m
$$

or [(2.22)](https://arxiv.org/pdf/0711.3575v2)

$$
\partial_t \alpha = \partial_t \alpha + k_g \alpha \mathcal{G}^i \\
$$

Where $\sigma$ > 0, and $k_g$ < 0. We'll be looking at the first case in this article, but both modifications are similar
...
Trying to simulate with the above set of modifications is not going to work, and instead today we're going to focus on trying to reproduce a specific result: todo. The no spin test case from [this paper](https://arxiv.org/pdf/gr-qc/0610128). Here, I'll lay out the test parameters:
...
The black holes are expected to collide at roughly T=160. For our sim, we'll have the following test parameters

```c++
resolution = {213, 213, 213}
simulation_width = 20
h = (simulation_width / (resolution.x - 1))
n (the damping parameter) = 2
todo: sigma
todo: other damping params
```

By the end of this article, we will be close enough to demonstrate that the sim we've built isn't totally invalid. Note that the boundaries here are sufficiently close that they'll cause problems quite quickly. Further away boundaries require higher resolutions in general

In general, exactly reproducing other people's results is an absolute nightmare, and is virtually impossible in the general case. While it isn't hard to get close, the field has a strong issue with being underdocumented, and many of the larger simulators simply do not publish enough information to replicate their results. Simulations also seemingly tend to involve a calibration step, but information on the limitations or inaccuracies of these simulations is nearly impossible to come by due to the nature of academic publishing. There are a large number of tweakable parameters and design choices, and there is rarely an explanation of why those were picked - despite being frequently quite important

I'm going to walk us through how to take our sim from failing quickly, to successfully completing over an orbit. We'll get 'close enough' to a result

## First off, lets test the standard modifications

With the sigma damping enabled, our sim goes from blowing up immediately, to successfully managing to simulate a collision at roughly T=CHECKME. Because we're more interested in the paths that the black holes take for the moment, rather than the specific collision time, I'm just eyeballing the numbers here

<iframe width="560" height="315" src="https://www.youtube.com/embed/oogqwuPbdm8?si=unCAuxZS7BhC-E5V" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
1. Constraint violations, and damping strength (including $\eta$ for the gauge damping parameter)
...
Testing this directly, our code immediately unconditionally blows up - likely because of our low grid resolution. It seems that for us, we do need some damping near the event horizon, and instead I clamp the minimum damping to $max(W, \epsilon)$
...
Our code now makes it to T=todo. One key feature to note is that: With very reduced kreiss oliger dissipation, we're now much more vulnerable to 0 speed modes (which are no longer dissipated away), and so you want to ensure you're using the advecting lapse and shift conditions for the gauge. I strongly suspect that shock avoiding gauge conditions would be of help here as well, but have not tested them

<iframe width="560" height="315" src="https://www.youtube.com/embed/XxRbxzmTqdE?si=i2dV0LWO2397sPHT" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
1. The $\sigma$ damping scheme tends to be the least robust, and the christoffel symbol constraint is often significantly incorrect. Its too simplistic and loses energy tremendously. For this reason it is frequently combined with other constraint enforcement techniques (eg replacing undifferentiated $\tilde{\Gamma}^i$s with the analytic variable), which we do not use as it is not a very general solution. It is, observationally, significantly lossy in terms of energy
2. This scheme works well, but isn't sufficiently able to damp away $\tilde{G}^i$ constraint errors, even with high values of the damping coefficient. Higher damping coefficients lead to lowering the courant factor (ie lowering your maximum timestep), which is undesirable
3. This scheme also works reasonably, but is also insufficiently damping

We're going to ditch the traditional $\sigma$ damping, and instead use the other two handling techniques combined, which together provide sufficient stability
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/YGj93N6omao?si=t28TmlW7xpwr3hDl" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/oizYRdPsNYM?si=Ht7aCrTSQxwRZbgt" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
