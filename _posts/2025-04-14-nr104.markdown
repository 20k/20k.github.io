...
All stars[^allstars] (other than black holes) in *hydrostatic equilibrium*. This means that their gravity is counterbalanced by an internal pressure. Surprisingly enough, this constraint, along with two other parameters, is enough to fully determine the structure of a star. We also need:
...
|-|-|-|-|-|-|-|
| [C1](https://arxiv.org/pdf/1606.04881) (section VII) | $6.235 \times 10^{17}\; \mathrm{kg}\;\mathrm{m}^{-3}$ |  $123.641 M_{\odot}^2$ where $c=G=1$[^waste] | $2$ |$1.543 M_\odot$ where $c=G=1$ | $13.4 \; \mathrm{km}$ in Isotropic coordinates | |
| [C2](https://einsteintoolkit.org/thornguide/EinsteinInitialData/TOVSolver/documentation.html) (see after interior) | $1.28 \times 10^{-3} \; \mathrm{m^{-2}}$ where $c=G=M_{\odot} = 1$ | $100$ where $c=G=M_{\odot} = 1$ | $2$ | $1.4 M_{\odot}$ where $c=G=1$ | $14.15\; \mathrm{km}$ where $c=G=1$ in schwarzschild coordinates | |
| [C3](https://arxiv.org/pdf/gr-qc/0110047) (see: C) | $8.0 \times 10^{-3}$ where $c=G=1$ | $100$ where $c=G=M_{\odot} = 1$ | $2$ | $1.447 M_{\odot}$ where $c=G=1$ | unclear | The paper mixes up rest and gravitational mass frequently |
...
This paper in theory lets us solve the TOV equations in a conformally flat spacetime, via (77-79), but we've deliberately skipping this step, as solving the TOV equations directly is much simpler. In general I wouldn't recommend implementing section VI (other than (83-84)), and I think (82) is incorrect unfortunately
...
Where $R$ is the radius of the star in schwarzschild coordinates, and $M$ is the total ADM mass (ie the integral of $m(r)$. $m(r)$ is still cumulative mass). $C$ is a constant. Technical details of how this works is available on the linked page[^linked], and its a good read if you're interested in how we get here
...
Implementing the extrinsic curvature is only mildly horrendous. First up, we need the ability to interpolate buffers by their isotropic radius, which inclues a boundary condition for the exterior of the star
...
auto get_AIJ = [&](v3f fpos)
{
    ...
    //interpolate all those functions we just calculated at the current isotropic radial coordinate
...
We'll need to plug some source $\bar{S}^i$ into the following equation to calculate the lorentz factor:
...
For this method, we have to calculate two individual sources - a linear part $\bar{S}^i_P$, and an angular part $\bar{S}^i_J$, and sum them together:

$$\begin{align}
\bar{S}^i_P &= P^i \sigma \\
\bar{S}^i_J &= \epsilon^{ijk} J_j x_k \kappa \\
\bar{S}^i_{summed} &= \bar{S}^i_P + \bar{S}^i_J
\end{align}
$$

where $\epsilon^{ijk}$ is the levi civita symbol, and $x_k$ is the world position relative to the centre of the neutron star
...
This is the method I've implemented, but there's an implementation of the former method as well lurking in the code
...
To get the rest mass density, and specific internal energy density, I apply the equation of state to recover them. Laying things out in memory here is a bit of a pain, but its to ensure that a single kernel has access to every equation of state per-star. If you're using one equation of state, and/or you're willing to invoke a separate kernel per-star, you could probably simplify this a lot. In the future however its likely that I'll be setting up 1000s of these things, so I like to keep my options open

That's it. We're done! We have our final ADM quantities, and our hydrodynamic quantities as well, so its mission accomplished
...
This next section is often left a bit underspecified, so here it is in detail: In GR, if you want to integrate something, you have to integrate it multiplied by the volume element. In GR, this is $\sqrt{|\mathrm{det}\; g|}$, where $g$ is the metric. This measures the volume of a parallelpiped, with dimensions $dx^0 dx^1 dx^2 dx^3$. However, we only actually want the volume element in the $r$ direction as we're integrating if we want to calculate the rest mass, which is $\sqrt{|(1 - 2\frac{m}{r})^{-1}|}$, or $(1 - 2\frac{m}{r})^{-\frac{1}{2}}$ here. This is how wikipedia gets its equation for gravitational binding energy
...
After writing this all, I decided to reach out to the author, which is something I never do. They said two things:

1. They state they have not done neutron stars with both spin and linear momentum
2. If you want spin, you should add the two $\bar{S}^i$ components together, and calculate the lorentz factor from (27)
