---
layout: post
title:  "Numerical Relativity 104: How do you build a neutron star from scratch?"
...
You should know before we get into this, that we're stepping into quite an involved process, and there's a lot of different equations to be solved. No individual step requires any advanced maths or solving techniques. So as long as we try and keep a clear oversight of the overarching process, we'll be fine
...
Fundamentally, a neutron star is a star that is so dense, that it is energetically favourable for the protons and neutrons that make it up to combine to form neutrons. As a result, they are made up largely of pure neutrons, under incredible pressure
...
2. There's no way to make general relativity and quantum physics get along nicely at these extreme scales. For context, they have an escape velocity of ~50% the speed of light
...
4. They have the most powerful magnetic fields of anything in the entire universe
...
First up: Let's build a static, non spinning neutron star. What we really want to determine is the distribution of matter within the star, so we can answer the question of its structure. The way that we do this is by calculating the pressure at each point, and then reverse engineering what kind of matter that pressure via the equation of state
...
In general, you'll need to numerically invert the equation of state for some arbitrary equation - which means its time to address equations of state more concretely
...
2. $K_{ij}$ is the extrinsic curvature, and is essentially a momentum term. When it is trace free ($\gamma^{ij}K_{ij} = K = 0$), it is often called $A_{ij}$
...
These components are then projected onto our hypersurface (ie, we're sticking this into the ADM formalism). Here, this paper actually only requires two parts of that projection, which are defined as follows[^follows]:
...
Where $W$ is a lorentz factor ($=-n_\mu u^\mu$), and $u^i$ is the spatial components of a fluid 4-velocity. These two variables fully encompass our fluid's state here, and *also* fully determine the structure of our spacetime. Calculating these is a major goal here
...
|Variable|Meaning| Solution method | Notes
|-|-|-|-|
...
|$\bar{S}^i$| The ADM projection of part of the stress-energy tensor, conformally transformed | Calculated from our physical parameters + TOV, requires some numerical integration (surprise!) | |

Anything with a bar over it is a conformal variable, and their indices are raised and lowered with the conformal metric. This is the same conformal decomposition as the [black hole initial conditions](https://20k.github.io/c++/2025/01/12/nr102.html#initial-conditions) we've used previously[^explicitly]
...
From here, its time to dig into the really fun bits - this is the main chunk of the paper. As with our black hole solution, we have two parameters we can specify: the ADM momentum $P^i$, and the ADM angular momentum $J^i$. These are broken up into two equations: (51), and (54) - which we simply sum together
...
For every star, we now have $\bar{\mu}$, $\bar{p}$, and $\bar{A}^{ij} = \bar{A}^{ij}_P + \bar{A}^{ij}_J$. We'll need to combine all those pieces into one, and then solve the final part of our puzzle (22):
