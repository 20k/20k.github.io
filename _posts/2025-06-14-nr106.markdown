...
$$
\begin{align}
\rho_H &= m u^0 \alpha W^3 \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
S_i &= m u_i W^3 \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
S_{ij} &= m \frac{u_i u_j}{u^0} \alpha^{-1} W^3 \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
\end{align}
$$

These are summed across all particles. Note that when we're evaluating these, these can be split up into two components: The contributions per-particle, and per-cell

$$
\begin{align}
s &= m u^0 \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
s_i &= m u_i \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
s_{ij} &= m \frac{u_i u_j}{u^0}\delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
\\
\rho_H &= \alpha W^3 s\\
S_i &= W^3 s_i\\
S_{ij} &= \alpha^{-1} W^3 s_{ij}\\
\end{align}
$$

This allows you to split up the summation of the $s$ terms, and factor out the redundant calculations involving $W$ and $\alpha$
...
But, surprisingly, this still returns the wrong answer. What gives?
...
$f_j$ represents the function value at the corresponding offset, eg $f_0$ is $f(x - x_0)$

A 3d polynomial can be constructed as follows:

$$L(x, y, z) = \sum^3_a \sum^3_b \sum^3_c f_{abc} l_a(x) l_b(y) l_c(z)$$

This is a large sum of 64 values. As you have already guessed, we can't implement this as written otherwise we'll end up with massive systematic rounding errors. We instead have to perform a pairwise matched summation of all values, to ensure exactly the same order of operations on both sides of the grid

```c++
template<typename T, typename... U>
inline
auto function_tricubic_particles(T&& func, v3f frac, v3i ifloored, U&&... args)
{
    using namespace single_source;
