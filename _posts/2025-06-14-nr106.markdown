...
# From time to time, C++ is not a very good programming language

Let's imagine we have a set of initial conditions that looks like this, for a basic test problem that's discretised in one dimension, with a grid size of $7$:

|Cell Index|0|1|2|3|4|5|6|
|-|-|-|-|-|-|-|-|
|Value|$a_0$|$b_0$|$c_0$|$d_0$|$c_0$|$b_0$|$a_0$|

We can clearly see that this data is symmetric about the cell with index $3$. After a single timestep into the future, we might expect our initial conditions to evolve to a new state:

|Cell Index|0|1|2|3|4|5|6|
|-|-|-|-|-|-|-|-|
|Value|$a_1$|$b_1$|$c_1$|$d_1$|$c_1$|$b_1$|$a_1$|

For any correct simulation, we expect that symmetry to be preserved. In virtually all simulations however, if you implement some debugging, you'll very immediately notice that this property does not hold up - and the two sides of the grid start to *increasingly* diverge. This is despite having the same initial conditions on both sides!
...
Generally, your initial conditions will come in one of two forms:

1. A local 3-velocity
2. A coordinate 4d velocity

### Local 3-velocity

Here, velocity is defined relative to the inertial frame of reference of the particle. This means that to calculate the 4-velocity, we need to
