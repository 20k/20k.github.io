...
In both General Relativity, and Newtonian physics, we deal with pointlike N-body particles. These are fully described by their mass, position, and velocity in both formalisms
...
Clearly, we'll need to transplant this into the ADM formalism, but for once the general relativistic side of this is straightforward
...
S_{ij} &= m \frac{u_iu_j}{u^0} \alpha^{-1} W^3 \delta^{(3)}(x^i - x^i_{\mathrm{p}})\\
\end{align}
$$
...
Note that all the quantities evaluated here need to be interpolated from the grid at the particle's position $x^i_\mathrm{p}$, which I'll get back to
