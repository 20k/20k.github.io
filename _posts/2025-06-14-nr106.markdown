...
2. The equations we'll be using for this article, and finding mistakes in some papers
3. The deep joys of the dirac delta function, and how to implement it correctly
4. Integration, and a guide on implementation techniques
5. Some miscellaneous performance techniques
...
Tracking this down can be a pain, and this article is going to be very pedantic about IEEE. Before we get to the particle dynamics, there are three major sources of asymmetry that can perturb a simulation:
...
A first order glance at how this could introduce asymmetry is straightforward: compilers interpret these expressions left to right, and so we get the following expression:
...
The rounding is done in a different order compared to `diff9`, which is why we get different results on both sides of the grid
...
1. The [ADM geodesic equations](https://arxiv.org/pdf/1611.07906), which you may remember from the rendering article
...
The third formalism is by far the best, for the following reasons

1. It does not involve the extrinsic curvature $\tilde{A}_{ij}$ compared to #1, which is hugely good for peformance. There are also only three evolution equations vs 4
2. The 4-momentum is a very poor choice of evolution variable. We'll be dealing with particles with *very* tiny masses, so with an extremely small 4-momentum
...
The particle's position is evolved as follows:

$$
\frac{dx^i}{dt} = -\beta^i + \frac{\gamma^{ij} u_j}{u^0}
$$

This formalism is simple enough that you could derive it by hand from the relation provided: $u^\mu \nabla_\mu u_\nu = 0$, which is the definition of a geodesic (todo: check terminology). We did actually derive the position equation ourselves in the last article! So far, so good

Note: You'll need to interpolate the quantities above based on the position $x^i$, using the symmetric interpolation

## Source terms

As in the prior article, matter interacts with the metric in general relativity via source terms. The full 4d stress energy tensor is:

# The Dirac Delta Function $\delta$

This article is going to contain a lot of information on the dirac delta function. The dirac delta function has a very obtuse and non useful definition:
...
For all intents and purposes, this definition can be largely disregarded. The more useful definition of the dirac delta function is as follows:
...
One trick we can do is as follows:

$$\sigma = \frac{\int_{x - \frac{dx}{2}}^{x + \frac{dx}{2}} \Delta(y) dy}{h}$$

Where $y$ is our integration variable between the bounds $x - \frac{dx}{2}$ and $x + \frac{dx}{2}$. This leads to an exact conservation of the dirac delta

This also generalises to $n$ dimensions: the integral becomes $n$ integrals, and you divide by $h^n$
