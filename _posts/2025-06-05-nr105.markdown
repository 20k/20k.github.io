...
Hi! Its finally time to do something truly spectacular, and smash some neutron stars together. We'll be discussing how relativistic hydrodynamics works, how it compares to regular hydrodynamics, and some of the various approaches present in the literature. I've also extensively documented a lot of the issues you'll run into in practice if you try and implement this kind of thing
...
2. Semi Lagrangian. This is what most programmers are familiar with - you have grid cells, and trace your velocity backward through the grid cells to find out where your fluid originated from
...
This is generally[^generally] broken up into three 1D problems (which are summed):
...
1. The non atmospheric approach isn't super good in general. Material gets deleted, resulting in mass loss of a few %
...
Now.. while its clear that a spinning neutron star is a lot more wobbly than a regular neutron star, its very hard to tell if its actually spinning or not. To do this, we're going to have to squeeze the ol' brain meat very hard

### Advecting arbitrary fields

This step is more for fun. Lets define an initial colour distribution ${r, g, b}$, and map it to our stars density as follows:
...
Creating an arbitrary[^arbitrary] initial colour distribution can be done either as a function, or if you're willing to put an absolutely cat-astrophic amount of implementation work in: as a texture. If you've been following this series, its pretty obvious what I'm going to put in here:

[^arbitrary]: I don't *think* an arbitrary colour distribution actually reproduces $\rho_*$ and the correct motion of an advected quantity, so its at best illustrative. A constant colour however would do. Ideally you'd calculate the flux of $\rho_*$, and then apply that to your colours in some fashion, but this step isn't too serious

\<iframe width="560" height="315" src="https://www.youtube.com/embed/5pi4XzE-keE?si=UekikwPEgi_n3mCy" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

mrow. I spent way too long on all this for no reason. Here's a less memey look at the spinning:
...
You can see that the star settles into a stable oblate shape (or at least, stable enough). This test uses a much longer linear viscosity timescale, of $T=800$
