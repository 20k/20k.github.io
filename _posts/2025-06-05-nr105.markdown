...
https://en.wikipedia.org/wiki/MUSCL_scheme do this

REDOING AGAIN

https://en.wikipedia.org/wiki/MUSCL_scheme

The basic form of a hydrodynamics scheme is as follows:

$$\frac{du}{dt}  = \frac{1}{\Delta x}( F(u^*_{i + 1/2}) - F(u^*_{i + 1/2}) )$$

$F$ is our flux, and $i$ is the grid cell. For us, $F_i = v_i q_i$. The half fluxes $F_{i - 1/2}$ and $F_{i + 1/2}$ are the complexity of hydrodynamics. $u^*$ is actually a function defined as follows:

$$u^*_{i + 1/2} = \mathrm{pick}(u^L_{i + 1/2}, u^R_{i + 1/2})\\
u^*_{i - 1/2} = \mathrm{pick}(u^L_{i - 1/2}, u^R_{i - 1/2})\\
$$

$$\begin{align}
u^L_{i + 1/2} &= u_i + 0.5 \phi(r_i)(u_{i + 1} - u_i)\\
u^R_{i + 1/2} &= u_{i + 1} - 0.5 \phi(r_{i + 1})(u_{i + 2} - u_{i + 1})\\
...
Next up, we need to go from our interface quantity $u_{i \pm 1/2}$, to $F_{i \pm 1/2}$
...
This is wrong. Need to do sigma again, and split out the half velocity, and the half quantity u

$$
\Delta x = x_{i + 1} - x_i\\
\\
F^n_{i - 1/2} = \frac{1}{\Delta t} \int^{t_{n+1}}_{t_n} f(x_{i-1/2}, t) dt \\
\frac{(t_{n + 1} - t_n)}{\Delta t} \frac{1}{2} (f(x_{i - 1/2}, t_n) + f(x_{i - 1/2}, t_{n+1}))\\
\frac{1}{2} (f(x_{i - 1/2}, t_n) + f(x_{i - 1/2}, t_{n+1}))\\
f(x_{i - 1/2}) = f_i + \sigma_i (x_{i - 1/2} - x_i) \\
f(x_{i - 1/2}, t_n ) = f_i^n - \sigma_i \Delta x / 2 \\
f(x_{i - 1/2}, t_{n+1} ) = f_i^{n+1} - \sigma_i \Delta x / 2 \\
\frac{1}{2} (f_i^n - \sigma_i \Delta x / 2 + f_i^{n+1} - \sigma_i \Delta x / 2)\\
F^n_{i - 1/2} = \frac{1}{2} (f_i^n+  f_i^{n+1} - \sigma_i \Delta x)\\
\\
\\
\\$$
...
where $\Delta x$ is the scale. This is the flux conserving form. Solving for the half grid cell flux at the boundary between our grid cells is the joy of hydrodynamics
...
$$x_i = x_{i - 1/2} + \frac{1}{2} \Delta x$$

Notating this as $F^n$ is a little sketchy and sometimes instead I've seen it be called $F^{n + 1/2}$, so keep this in mind
...
## Cracking out the integral

One feature of many derivations is that it assumes that the velocity in the integral above is constant, or that we have to perform a very rough approximation. Because we'll be implicitly solving this equation, we'll have our velocities both at the current time $n$, and the next timestep $n+1$

The common approximation made is as follows:

$$q^n(x, t_{n+1}) = q^n(x - v \Delta t, t_n)$$

But we have the quantity on the left hand side available in later steps. We can consider a general piecewise limited flux scheme to be as follows:

https://www.tevza.org/home/course/modelling-II_2016/books/Leveque%20-%20Finite%20Volume%20Methods%20for%20Hyperbolic%20Equations.pdf 6.10

$$F_{i - 1/2} = F_L(q_{i - 1}, q_i) + \phi^n_{i - 1/2}(F_H(q_{i-1}, q_i) - F_L(q_i-1, q_i))$$

Note that this is only one side of the advection equation, and I'll spell this out in full (todo?)

If we want to calculate $q(x, t_n)$, we have to piecewise interpolate as follows:
...
The way that this works is: if our velocity is > 0, we take the right limit of the left interface. If our velocity is < 0, we take the left limit of the right interface. This is confusing, so here's what it looks like:

$$
v^{n + 1/2}_{i - 1/2} > 0\\
F^n_{i - 1/2} = \frac{1}{2} (f_{i-1}^n +  f_{i-1}^{n+1} + \sigma_{i-1} \Delta x)\\
v^{n + 1/2}_{i - 1/2} < 0\\
F^n_{i - 1/2} = \frac{1}{2} (f_{i}^n +  f_{i}^{n+1} - \sigma_i \Delta x)\\
$$
...
1. $v^{n + 1/2}_{i - 1/2} = \frac{v^{n + 1/2}_{i} + v^{n + 1/2}_{i - 1}}{2}$

## A specific scheme: MUSCL

This scheme swaps betwen second order, and first order, by using a limiter function on how much fluid can be transported from one cell to the next. Different limiters have different properties - some introduce lots of diffusion, some accidentally introduce oscillations. The most common, least diffusive limiter, appears to be known as superbee. Todo: Superbee

There's two different forms of this equation, slope limited, and flux limited

Todo: I might be able to give a compositional form between an advected scheme, and this flux limited scheme

### Slope limited

https://arxiv.org/pdf/1705.10608

The 2nd order numerical flux has the general form:
...
There's also the flux limited form:
...
$\phi$ is a *flux* limiter. There's nothing different here to the slope limiting equations, its just a different way of limiting stuff. We'll be using the suerbee limiter:
