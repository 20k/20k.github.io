...
Todo: Redo this segment, do this:

https://repository.gatech.edu/server/api/core/bitstreams/f5e116da-ebff-4380-96e8-027304d9db82/content

Finding good test cases is tricky. Because we're mainly visually debuggings things so far, we don't produce gravitational waveforms to match against other results. On top of that, it seems that the literature around neutron stars isn't quite as explicit with the initial conditions and testable results as it is for black holes, so there's often quantities which are left unspecified
...
Without an exact value for linear momentum, we cannot precisely attempt to replicate the original paper, but we appear to get a reasonable inspiral. Its a bit unsatisfying

Overall, this scheme does appear to work. Exactly how energy conserving it is is up for grabs, but there's nothing obviously unphysical going on. The hamiltonian constraint errors appear to stay low and bounded which is great as well. More resolution or a coordinate transform would likely help here significantly too, but that's a topic for the future
