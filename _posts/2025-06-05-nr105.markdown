...
If you're like me, you might not even really know why hydrodynamics is a separate area of inquiry to just solving PDEs in general. Or, you might just think its an umbrella term for people who apply the same techniques we've been using already, but simply to fluids
...
\partial_t p_* &+ \nabla  (p_* \textbf{v}) = 0\\
\partial_t e_* &+ \nabla  (e_* \textbf{v}) = 0\\
\partial_t \tilde{S}_k &+ \nabla  (\tilde{S}_k \textbf{v}) = F\\
\end{align}$$

This is the standard representation in hydrodynamics of advection equations, with $F$ being a source term. You see, in hydrodynamics there are things called *conserved quantities*. We might imagine these to be rest mass, energy, and momentum for a fluid dynamics simulation. The idea behind a conserved quantity is that we don't want it to change, ie we want the total rest mass to remain the same
...
This kind of discretisation makes no guarantees about conservation. While it can be made to work, in general the hydro people will be very angry at you, and your simulation will be bad quality. What's the alternative?
