...
Its finally time to do something truly spectacular, and smash some neutron stars together. We'll be discussing how relativistic hydrodynamics works, how it compares to regular hydrodynamics, and examining a specific approach in the literature. I've also extensively documented many of the issues you'll run into in practice if you try and implement this kind of thing
...
We can transform these to a form which is more familiar to regular hydrodynamics:
...
In hydrodynamics there are things called *conserved quantities* - like rest mass, energy, and momentum for a fluid dynamics simulation. The idea behind a conserved quantity is that eg a good simulation should keep the total rest mass exact, instead of drifting
...
This kind of discretisation makes no guarantees about conservation. While it will work, in general the hydro people will start to make very unhappy noises, and your simulation will be physically very suspect. What's the alternative?
...
In numerical relativity, 'shocks' are partially handled via Kreiss-Oliger dissipation[^ramble] - but the equations don't generally produce the kinds of situations[^shocks] you get with hydrodynamics. Kreiss-Oliger isn't really appropriate to hydrodynamics however, as it won't conserve our variables
...
where $\Delta x$ is the scale. This is the flux conserving form, and it is generic across any hydrodynamics scheme. Solving for the half grid cell flux at the boundary between our grid cells is the joy of hydrodynamics
...
Todo: The following paragraph is a bit imprecise. Its an issue with riemann solving
...
We can solve #1 by using fixed point iteration with the initial guess $w=\rho_*$[^anyguess] as this is a lorentz factor multiplied by density terms. Simply plug in your guess into the right hand side, calculate the new value of $w$, and rinse and repeat. #2 is more of a pain
...
Iterating $w$ is extremely cheap. $140$ iterations is way more than necessary here. Because we can get away with such a high number of iterations, I apply a small amount of relaxation to the solution - just to make sure that this function can never cause us any problems
...
Despite our equations looking like a conservative hydrodynamics equation, they actually are not in this formalism. While I believe that $\rho_*$ *is* in conservative form, $e_*$ is not and is not a conserved quantity across shocks - so we'll have to artificially dissipate them despite using a shock capturing formalism. I have absolutely no idea about $\tilde{S}_k$. We'll be getting to numerical dissipation soon
...
2. We have a new source term for $e^*$ (involving $Q$)
...
Q_{Lvis} =
\begin{cases}
-C_{Lvis}\sqrt{(\Gamma/n) \rho_* A}\;\;&\mathrm{for} \;\delta v < 0\\
0 \; &\mathrm{otherwise}
\end{cases}
\end{align}$$
...
The term $(\rho_0 \epsilon)^{-1 + \frac{1}{\Gamma}}$ is numerically bad to calculate directly, and really you should calculate this term as:
...
In general, linear viscosity is good for large scale damping and significantly improves the simulations stability. Our initial conditions are quite approximate, and the stars oscillate while relaxing into their final form - and linear visocosity damps those oscillations. Quadratic viscosity seems to be less applicable here, and hasn't produced significant changes to the simulation, but it also may be just the tests that I've performed
...
There's nothing terribly exciting here. There's an option to not evolve the right hand sides when $\rho_*$ is low for experimental purposes, which does make this a bit messy. The right hand side of $e_*$ is also not evolved under the same conditions that $Q$ is not calculated, to avoid NaNs propagating

## ADM Source terms

Now that the evolution equations are done, we need to figure out how to integrate this into the BSSN equations. The way matter terms work is via something known as source terms - these make up the components of the stress-energy tensor - which up until now, has been 0. This is fed into our BSSN equations, to enable matter to affect spacetime

There are four standard ADM source terms, representing the ADM projections of the 4D stress-energy tensor. We can calculate them from our matter variables as follows:
...
We'll have to modify our BSSN source term to match the absorbed $W^2$ term (ie $-8 \pi \alpha S_{ij}$), but now our source terms become regular at the metric tensor. This is a free win, at the expense of non standard notation that we'll have to keep in mind into the future
...
2. When $\rho_*$ is of the same order of magnitude as $\rho_{min}$, you have to be extremely careful with floating point accuracy. There's a significant amount of error due to precision issues, and some quantities (like visocosity) are poorly behaved
...
We're really *really* pushing the limits of single precision with this technique as well, its right on the boundary of what is acceptable: if you're interested in novel precision techniques, like unums, or projective reals, this would be a great playground

Its clear though seeing that we easily can get multiple orbits, collapse neutron stars into stable black holes, and produce visible gravitational waves, that this is working pretty well. So I'm very happy with it overall, and its a great base to build off of

# Next time

The next article is going to be N-body particle dynamics, because that should be a lot easier than hydrodynamics. It'll be great!
...
$k_m = 0.04 * \mathrm{should\_damp}$, as described earlier
...
Where $\chi = 0.9$, $k_{\gamma} = -0.18$, $C = 0.15$, and $\mathrm{step}(x) = x \geq 0 \;?\; 1 : 0$
...
todo: mention visco instability
