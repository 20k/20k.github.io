...
## Flux conservative discretisation
...
$_i$ is a grid cell offset - I'm not going to try and fight hydro notation, its just going to clash terribly with tensor index notation and we'll live with it. The conservative discretisation scheme is then as follows:
...
There's two different forms of these equations, slope limited, and flux limited
...
$\phi$ is a *flux* limiter. There's nothing fundamentally that different here to the slope limiting equations - this form seems to be largely to avoid the selection operator in the slope limited version. We'll be using the superbee limiter:
...
In general, these options seem to be fairly identical, so you may as well use the first

If you're wondering why we don't just reconstruct the flux directly, and limit that without using a half velocity at all: The answer seems to be because of the upwinding scheme. If you try and approach like this, the reconstructed flux's velocity, and the velocity you use to toggle the upwind direction will differ - which leads to an incorrect solution

More advanced solvers I believe use a Riemann solver instead of upwinding (?), which renders this unnecessary

## Relativistic hydrodynamics

In theory, the hydrodynamics is now done

Here's where there's a small kick in the teeth: Despite our equations looking like a conservative hydrodynamics equation, they actually are not. While I believe that $p_*$ *is* in conservative form, $e_*$ is not and is not a conserved quantity across shocks - so we'll have to artificially dissipate them despite this being a shock capturing formalism. I have absolutely no idea about $\tilde{S}_k$

In general, this is one of the reasons why this formalism was abandoned - more modern formalisms are expressed purely in terms of conserved quantities, which makes their hydro treatment more theoretically rigorous. I'll get more to this point in the discussion

# Evolution Equations

With that, we can implement the above scheme for all the $\partial_i(q v^i)$ terms, which works great. There's two other nontrivial terms to talk about now: $v^i$ and $w$
...
There's a minor problem: $S_{ij}$ is singular at the singularity. The issue is that $\gamma^{ij}$ is undefined
...
We'll have to modify our BSSN source term to match the absorbed $W^2$ term (ie $\tilde{\gamma}^{ij} S_{ij}$), but now our source terms become regular at the metric tensor. This is a minor improvement over the standard arrangement
...
One of the big problems with our equations is that they actually aren't conservative. When I picked these equations I wasn't super aware of the importance of this: one key thing is that $e^*$ isn't actually conserved in the presence of shocks. To combat this, this scheme introduces artificial viscosity - luckily, in a fairly small way
...
$$\begin{align}
\begin{cases}
Q_{Qvis} = C_{Qvis}A(\delta v)^2\;\;&\mathrm{for} \;\delta v < 0\\
0 \; &\mathrm{otherwise}
\end{cases}
\end{align}$$
...
Quadratic viscosity is generally left on throughout a simulation, but is disabled near a singularity

## Linear viscosity

$$\begin{align}
\begin{cases}
Q_{Lvis} = -C_{Lvis}\sqrt{(\Gamma/n) p_* A}\;\;&\mathrm{for} \;\delta v < 0\\
0 \; &\mathrm{otherwise}
\end{cases}
\end{align}$$
...
I don't have a clue what $n$ is and have never been able to find a single piece of discussion on it, so I set it to $1$

## Viscosity $e_*$

When using viscosity, you have to modify the evolution equation of $e_*$. The term to add is:

$$\partial_t e_* += -(\rho_0 \epsilon)^{(-1 + \frac{1}{\Gamma})} \;\; \frac{Q}{\Gamma} \; \partial_k (\frac{w W^{-3} v^k}{p_*})$$

## Viscosity conclusion

In general, linear viscosity is good for large scale damping and significantly improves the simulations stability. Our initial conditions are quite approximate, so damping this initial relaxation improves the simulation a lot. Quadratic viscosity is less applicable, as shocks aren't common in these simulations and so it has minimal effect - I also have a feeling that the much higher resolution, and somewhat more modern hydro approach significantly helps here
...
One of the key aspects of this paper is the non-atmospheric aspect of it. The idea here is that when $\rho_* < \rho_{min}$, you flush all your evolution variables to $0$ for a true vacuum. This is a tad unusual as hydrodynamics goes, but it does leave a few questions about exactly how and when to flush your evolution variables to zero
...
In general, the lapse $\alpha$ is a good surrogate variable for being near an event horizon. I use a conservative $0.45$ for disabling viscosity, and $0.15$ for dissipating matter fields. The latter constraint imposes a minimum simulation resolution to successfully simulate a black hole collapse
...
If you're not using a finite-volume/conservative hydrodynamics scheme, one issue you can run into is that $v^i$ can easily blow up, via $\tilde{S}_k$. Clamping the maximum velocity of the fluid $u_k$ can fix this, but the real solution is to implement an actual hydro scheme
...
$C$ is a damping strength, set to $0.15$. It is $0$ when the distance to the simulation's boundary is $<10$ in grid cells. The prefix term of $C \Delta t$ is derived from [this](https://arxiv.org/pdf/2404.01137) paper, note that we're using $\mathcal{H}_-$ in that paper's notation
...
The near-boundary modification has the effect of significantly reducing constraint contamination of the simulation, compare below: todo
...
Implementign this correctly is extremely difficult to do in a standard programming language in a high performance way, because the BSSN equation implementation itself needs generic, direct access to whatever buffers can derive our source terms (including $w$)
...
The vast majority of our grid has no fluid in it, so simulating the evolution equations there is completely pointless. Because we're limited solely by memory bandwidth - if we early-out via some condition, we can avoid any extra memory reads, and write out only $0$s, which is pretty great in general
