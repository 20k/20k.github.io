...
| $h$ | $1 + \epsilon + \frac{P}{\rho_0}$ | $1 + \Gamma \epsilon$ with the perfect fluid equation of state |
|$w$ | $\rho_* \alpha u^0$ | Must be calculated via an iterative procedure |
|$v^i$ | $\frac{u^i}{u^0}$ | Represents a coordinate velocity |

The notation in this article is fully harmonised with the last one. $\rho_0$ is the rest-mass density, $\epsilon$ is the specific energy density, $u^0$ is the lorentz factor, and $\alpha$ is the lapse
