...
2. Semi Lagrangian. This is what most programmers are familiar with - you have grid cells, and trace your velocity backward through the grid to find out where your fluid originated from (and what properties it had)
3. Eularian. Here, we have grid cells, and only deal with quantities locally in terms of their values and derivatives

Today we'll be using a purely Eularian scheme. We'll get to the differences between the hydrodynamic techniques for solving Eularian equations, and regular differential equation solving, after we get to the relativistic portions of this article
...
|$e_*$ | $(\rho_0 \epsilon)^\frac{1}{\Gamma} \alpha u^0 W^{-3}$ | Unclear how a numerical equation of state would work, or if a piecewise $\Gamma$ is valid. Represents energy |
...
In the future, we'll have to calculate eigenvectors to produce a full Riemann solver when we get around to more advanced hydrodynamics
...
Papers here can be quite vague as to how exactly to modify the viscosity. A complete description of this scheme requires us to go all the way back to [1984](https://adsabs.harvard.edu/full/1984ApJS...55..211H), where the extra viscous pressure is called $Q$. This is used in two places:
...
$t$ is the total elapsed simulation time, $K$ is a constant (which I set to $0.1$), and $T$ is a constant that represents a damping timescale for the linear viscosity, as it is generally only useful in the early parts of the simulation to iron out the star's oscillations. Leaving it on too long will result in a loss of velocity, and $T=200$ seems to work well
...
There's nothing terribly exciting here. There's an option to not evolve the right hand sides when $\rho_*$ is low for experimental purposes. The right hand side of $e_*$ is also not evolved under the same conditions that $Q$ is not calculated
...
In this form, they're less useful than they could be: $S_{ij}$ is singular at the singularity, as $\gamma_{ij}$ is undefined
...
Without an exact value for linear momentum, we cannot precisely attempt to replicate the original paper, but we appear to get a reasonable inspiral. Its a little unsatisfying - I've been searching around for test cases which give exact values, but in general people tend to miss out one or more key parameters. Within a few articles we'll be producing gravitational waves together, which is likely when I'll be investing in a post newtonian expansion + energy minimisation scheme, as that makes this very directly testable. This is a much better approach than trying to replicate things by staring at graphs in papers, because we can quantitively determine the accuracy then
...
The next article is going to be N-body particle dynamics, because this hydrodynamics implementation took a tremendous amount of work, and I need to go do something incredibly silly and fun. It'll be great!
