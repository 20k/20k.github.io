...
In general, the amount of fluid that can be transported from one cell to the next is limited in some fashion. Different limiters have different properties - some introduce lots of diffusion, some accidentally introduce oscillations.
...
This is the *slope* limited form, where $\sigma_i$ is a a slope limiter. There's also the flux limited form:
...
$\phi$ is a *flux* limiter. There's nothing different here to the slope limiting equations, its just a different way of limiting stuff
