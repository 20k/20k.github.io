...
valuef calculate_w(valuef p_star, valuef e_star, valuef W, inverse_metric<valuef, 3, 3> icY, v3f Si)
{
    ...
    //140 iterations out to be enough for anybody
    for(int i=0; i < 140; i++)
    {
...
The way matter integrates into the BSSN equations is via something known as source terms. These make up the components of the stress-energy tensor - which up until now, has been 0. This is fed into our BSSN equations, to enable matter to affect spacetime
...
In this form, they're less useful than they could be: $S_{ij}$ is singular at the singularity. The issue is that $\gamma_{ij}$ is undefined
