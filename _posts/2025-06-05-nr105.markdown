...
$\phi$ is a *flux* limiter. There's nothing fundamentally that different here to the slope limiting equations, its just a different way of limiting stuff. We'll be using the superbee limiter:
...
2. $v^{n}_{i - 1/2} = \frac{p^*_i v_i + p^*_{i-1} v_{i-1}}{p^*_i + p^*_{i-1}}$

$p^*$ is our rest mass. I've also tested a second order interpolation

In general, these options seem to all be the same
