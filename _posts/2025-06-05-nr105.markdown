...
Finding good test cases is tricky. A particular set of initial conditions in the literature is very widely used, and I'm not incredibly sure how to translate it to our initial condition set - or if it'd be valid to do so

There's a few issues:

1. The total ADM mass of the system is dependent on the spin, and momentum of the neutron stars. The rest mass is also nonlinearly dependent on the initial conditions, because of the hamiltonian correction
2. Can you directly convert a quantity like an orbital period, into an angular momentum in general relativity? We're clearly deep into the relativistic regime, so it may not help us

With this in mind, the two tests are to check that things are working well enough. I've deliberately used a neutron star that we've have previously, to help cross checking

| Name | $\rho_c$ | $K$ | $\Gamma$ | Central Separation ($\mathrm{km}$) | Linear momentum | $\chi$ | Notes |
|-|-|-|-|-|-|-|-|
|I1, Similar to [A.](https://arxiv.org/pdf/1606.04881) | $6.235 \times 10^{17} \; \mathrm{kg}\;{m^{-3}}$| $123.641$ | $2$ | $54.6 \mathrm{km}$ | $\pm 0.25$ | $0$ | |
|I2| $3.235 \times 10^{17} \; \mathrm{kg}\;{m^{-3}}$| $123.641$ | $2$ | $54.6 \mathrm{km}$ | $\pm 0.14$ | $0.25$, perpendicular to the orbital plane (and cospinning) | |

https://arxiv.org/pdf/1311.4443

0.000960296 p0c

Don't just list M like a butt even though you'll get to this point in the article and go mmkay but what if i just did that though
