...
$N$ is generally between $0.05$ and $2$, and is technically $\frac{2}{M}$ where $M$ is the total ADM mass of the system
...
$$\begin{align}
\partial_t \tilde{A}_{ij} \;+= k_m \alpha \tilde{D}_{(i}\mathcal{M}_{j)} \\
\end{align}$$
...
\\
\partial_t \Gamma^i\; &+=\; - (\chi s + \frac{2}{3}) \mathcal{G}^i \partial_m \beta^m\\
\partial_t W \; &\mathrel{+}= \; C \; \Delta t \;\alpha W \mathcal{H}\\
\end{align}
$$

$$\begin{align}
\partial_t \tilde{\gamma}_{ij} \; += \;k_{\gamma} \alpha \tilde{\gamma}_{k(i} \tilde{D}_{j)} \mathcal{G}^k
\end{align}$$
...
$$\partial_t \alpha \; += \; - W (h e^{-\frac{t^2}{2 \sigma^2}}) (\alpha - W)$$
