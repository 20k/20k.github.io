...
In numerical relativity, 'shocks' are handled via Kreiss-Oliger dissipation[^ramble], but this isn't a conservative solution. This is fine there, but will result in hydro people making frowny faces at you. Its more socially acceptable to smooth out spacetime - less so a fluid
...
where $\Delta x$ is the scale. This is the flux conserving form. Solving for the *half* grid cell points at the boundary between our grid cells, is the joy of hydrodynamics

One really interesting fact is that its not possible to have an unconditionally stable method of higher than first order, spatially. All high resolution hydro methods mix between first order when near shocks, and higher order in smooth areas
...
Where $\sigma_i$ is a *slope limiter*. We'll be back to this
